...
class LDA_VI:
    def __init__(self, alpha, eta, K, source, whether_source, _lambda=1,  eta_seed=None, eta_not_seed=None, seed_words=None,
        ...
        self.whether_source = whether_source
    ...
    def _init_params(self, X, cv):
        ...
        if self.confirmatory:
            ...
            if self.two_phase_words is None:
                nums_topic = self.K
            else:
                nums_topic = self.K-1

            for k in range(nums_topic):
                setdiff_index = np.array(list(set(range(nums_topic)) - set([k])))
        ...
        if self.two_phase_words is not None:
            # use only words in corpus
            self.two_phase_words = [w for w in  self.two_phase_words if w in self.w2idx.keys()]

            self.two_phase_words_idx = [self.w2idx[w] for w in self.two_phase_words]

            # low prior on auxiliary topic on seed words other topic
            for key in self.seed_words.keys():
                self.eta[self.K-1, np.array(self.seed_words[key])] = self.eta_not_seed

            # low prior on overlapping words on other topic
            for k in range(nums_topic):
                self.eta[k, np.array(self.two_phase_words_idx)] = self.eta_not_seed

            # high prior on auxiliary topic on overlapping words
            self.eta[self.K - 1, np.array(self.two_phase_words_idx)] = self.eta_seed
        ...
        # make asseymmetric prior for word-topic distribution
        # different by each topic
        self.eta = 0.0001 * np.ones((self.K, self.V))

        # using source distribution
        print(f'Topics from source distribution are {list(self.source.keys())}')
        topic_idx = 0

        for topic in self.source.keys():
            w2c = self.source[topic]
            overlapping_words = 0
            for word in w2c.keys():
                ...
                self.eta[topic_idx, idx] = count**(self._lambda)

            topic_idx += 1
            print(f'Number of overlapping words with given corpus: {topic}: {overlapping_words}')
    ...
    def _loglikelihood(self, prior, distr, Edirichlet, size, beta=None, theta=None):
        ...
        if beta and self.whether_source:
    ...
    def _approx_bound(self, X):
        ...
        # E[ log p(z | theta) - log q(z | phi) ]
        temp = ( Elogtheta[np.newaxis,:] - np.log(phi+EPS) ) * phi
        ELBO += temp.sum().sum()
        ...
        # E[log p(beta | eta) - log q (beta | lambda)]
        if self.confirmatory or self.whether_source:
